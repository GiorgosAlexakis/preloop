...
class TrackerClient:
    ...
    def scan_projects(self, db: Session, organization: Organization) -> List[Project]:
        ...
        for proj_data in proj_data_list:
            ...
            if project:
                # Update existing
                project = crud_project.update(db, db_obj=project, obj_in=proj_create_data)
...
def scan_tracker(db: Session, tracker: Tracker, verbose: bool = False) -> Dict[str, Any]:
    """
...
def scan_all_accounts(db: Session, verbose: bool = False) -> Dict[str, Any]:
    ...
    logger.info(f"Processed {overall_stats['issues']} issues, updated {overall_stats['embeddings_updated']} embeddings")
