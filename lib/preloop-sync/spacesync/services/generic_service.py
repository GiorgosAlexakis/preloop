...
class GenericPollingUpdateService(PollingTrackerUpdateService):
    ...
    def setup(self) -> bool:
        ...
        logger.debug(
            f"GenericPollingUpdateService setup complete for tracker {self.tracker.id}"
        )
        return True
    ...
    def update(self) -> int:
        ...
        """
        logger.info(
            f"Starting update poll for tracker {self.tracker.id} ({self.tracker.name})"
        )
        total_embedding_updates = 0
        rate_limited_tracker = (
            False  # Flag to stop processing this tracker if rate limited
        )
        ...
        try:
            ...
            if not db_organizations:
                logger.info(
                    f"No active organizations found for tracker {self.tracker.id}. Skipping update cycle."
                )
                return 0
        except Exception as e:
            logger.error(
                f"Failed to get organizations for tracker {self.tracker.id}: {e}",
                exc_info=True,
            )
        ...
        for org in db_organizations:
            if rate_limited_tracker:
                logger.warning(
                    f"Skipping remaining organizations for tracker {self.tracker.id} due to rate limit."
                )
                break  # Stop processing orgs for this tracker if rate limited

            logger.debug(
                f"Processing organization {org.identifier} (ID: {org.id}) for tracker {self.tracker.id}"
            )
            ...
            try:
                ...
                processed_projects = self.client.scan_projects(
                    db=self.db, organization=org
                )
                logger.info(
                    f"Successfully scanned/synchronized {len(processed_projects)} projects for org {org.identifier} (tracker {self.tracker.id})."
                )
            except TrackerRateLimitError as rle:
                logger.warning(
                    f"Rate limit hit for tracker {self.tracker.id} during project scan for org {org.identifier}. Pausing updates for this tracker. Details: {rle}"
                )
                ...
                logger.error(
                    f"Tracker error scanning projects for org {org.identifier} (tracker {self.tracker.id}): {te}",
                    exc_info=True,
                )
                continue  # Skip this org
            except Exception as e:
                logger.error(
                    f"Unexpected error scanning projects for org {org.identifier} (tracker {self.tracker.id}): {e}",
                    exc_info=True,
                )
                continue  # Skip this org
            ...
            logger.info(
                f"Scanning issues for {len(processed_projects)} projects in org {org.identifier} (tracker {self.tracker.id})."
            )
            for project in processed_projects:
                if rate_limited_tracker:
                    logger.warning(
                        f"Skipping issue scan for project {project.identifier} due to rate limit."
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"Rate limit hit for tracker {self.tracker.id} while scanning project {project.id}. Pausing updates for this tracker. Details: {rle}"
                    )
                    rate_limited_tracker = True  # Set flag
                    break  # Stop processing projects for this org
                except TrackerError as te:
                    logger.error(
                        f"Tracker error scanning issues for project {project.id} (tracker {self.tracker.id}): {te}",
                        exc_info=True,
                    )
                    continue  # Continue with next project
                except Exception as e:
                    logger.error(
                        f"Unexpected error scanning issues for project {project.id} (tracker {self.tracker.id}): {e}",
                        exc_info=True,
                    )
                    continue  # Continue with next project

        logger.info(
            f"Finished update poll for tracker {self.tracker.id}. Total embedding updates: {total_embedding_updates}. Rate limited: {rate_limited_tracker}"
        )
    ...
    def cleanup(self) -> None:
        ...
        logger.info(
            f"Cleaning up generic polling service for tracker {self.tracker.id}"
        )
        pass
