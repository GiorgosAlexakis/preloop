...
class TrackerUpdateServiceManager:
    ...
    def __init__(self, db: Session = None, max_workers: int = 10, reload_interval: int = 300):
        """
    ...
    def _start_service(self, tracker: Tracker):
        ...
        if service.setup():
            ...
            logger.error(f"Failed to set up service for tracker {tracker_id} ({tracker.name})")
        ...
        while self.running:
            ...
            for tracker_id in list(self.services.keys()):
                if tracker_id not in active_tracker_ids:
                    logger.info(f"Tracker {tracker_id} is no longer active, stopping service")
                    self._stop_service(tracker_id)
            ...
            for tracker in active_trackers:
                tracker_id = str(tracker.id)
                if tracker_id not in self.services:
                    logger.info(f"Found new active tracker {tracker_id}, starting service")
                    self._start_service(tracker)
