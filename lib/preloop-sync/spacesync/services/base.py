...
from ..config import logger, SERVICE_WEBHOOK_ENABLED
...
class PollingTrackerUpdateService(BaseTrackerUpdateService):
    ...
    def poll_loop(self) -> None:
        ...
        """
        logger.info(f"Starting polling for tracker {self.tracker.id} ({self.tracker.name})")
        
        while self.running:
            try:
                updates = self.update()
                logger.info(f"Polled tracker {self.tracker.id}, found {updates} updates")
                self.last_check = datetime.datetime.utcnow()
    ...
    def stop(self) -> None:
        ...
        if self.thread and self.thread.is_alive():
            self.thread.join(timeout=5.0)  # Wait up to 5 seconds for thread to terminate
