...
from ..config import logger, SERVICE_HOST, SERVICE_PORT
...
class GitLabWebhookUpdateService(WebhookTrackerUpdateService):
    ...
    def __init__(self, db: Session, tracker: Tracker, host: str = SERVICE_HOST, port: int = SERVICE_PORT):
        """
    ...
    def setup_routes(self) -> None:
        ...
        def webhook_handler(tracker_id):
            ...
            if not self._verify_webhook_signature(request):
                logger.warning(f"Invalid webhook signature for tracker {self.tracker.id}")
            ...
            try:
                ...
                logger.error(f"Error processing webhook for tracker {self.tracker.id}: {str(e)}")
    ...
    def _handle_issue_event(self, payload: Dict[str, Any]) -> None:
        ...
        logger.warning(f"Could not find project with identifier {project_id} for tracker {self.tracker.id}")
    
    def _update_issue(self, organization: Organization, project: Project, issue_id: str) -> None:
        """
        ...
        try:
            ...
            issue_create_data = self.client.client.transform_issue(issue_data, project.id)
            ...
            if issue:
                ...
                if (issue.title != issue_create_data["title"] or 
                    issue.description != issue_create_data["description"]):
                    content_changed = True
                
                # Update existing
                issue = crud_issue.update(self.db, db_obj=issue, obj_in=issue_create_data)
            ...
            logger.error(f"Error updating issue {issue_id} for project {project.id}: {str(e)}")
    
    def _init_gitlab_client(self) -> None:
        """Initialize the GitLab client."""
        if not self.gitlab_client:
            connection_details = self.tracker.connection_details or {}
            gitlab_url = connection_details.get('gitlab_url', 'https://gitlab.com')
            
            self.gitlab_client = gitlab.Gitlab(
                url=gitlab_url,
                private_token=self.tracker.api_key
            )
