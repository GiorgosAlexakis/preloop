...
class TestJiraTrackerWebhooks(unittest.TestCase):
    ...
    def test_register_webhook_success(self):
        ...
        self.mock_crud_webhook.get_by_project_id.assert_called_once_with(self.mock_db_session, project_id="proj-db-id")
    ...
    def test_register_webhook_already_in_db(self):
        ...
        self.mock_crud_webhook.get_by_project_id.return_value = Webhook(id="wh-id", project_id="proj-db-id", external_id="123")
    ...
    def test_unregister_webhook_success(self):
        ...
        mock_webhook = Webhook(id="wh-db-id", project_id="proj-db-id", external_id="12345")
        ...
        result = self.tracker.unregister_webhook(db=self.mock_db_session, project_id="proj-db-id")

        self.assertTrue(result)
        self.mock_crud_webhook.get_by_project_id.assert_called_once_with(self.mock_db_session, project_id="proj-db-id")
        self.mock_jira_client._session.delete.assert_called_once_with("https://myjira.atlassian.net/rest/webhooks/1.0/webhook/12345")
        self.mock_crud_webhook.remove.assert_called_once_with(self.mock_db_session, id="wh-db-id")
    ...
    def test_unregister_webhook_not_in_jira(self):
        ...
        mock_webhook = Webhook(id="wh-db-id", project_id="proj-db-id", external_id="12345")
        self.mock_crud_webhook.get_by_project_id.return_value = mock_webhook
        self.mock_jira_client._session.delete.side_effect = JIRAError(status_code=404, text="Not Found")
