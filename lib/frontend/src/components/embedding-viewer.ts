...
export class EmbeddingViewer extends LitElement {
    ...
    @state() private projectColorMap = new Map<string, [number, number, number]>();
    ...
    firstUpdated() {
        const canvas = this.shadowRoot?.querySelector('#deck-canvas') as HTMLCanvasElement;
        this.deck = new Deck({
            canvas,
            initialViewState: {
                longitude: 0,
    ...
    private async fetchAndVisualize() {
        ...
        const ids = this.projectIds.split(',').map(id => id.trim()).filter(id => id);
        ...
        try {
            ...
            const validData = allEmbeddings.filter(item => item && item.embedding && Array.isArray(item.embedding) && item.embedding.length > 0);
            ...
            const uniqueProjectIds = [...new Set(validData.map(item => item.project_id))];
            ...
            if (validData.length === 1) {
                ...
                this.tsneWorker.postMessage({ vectors, options, iterations: TSNE_ITERATIONS });
            }
        } catch (error) {
    ...
    private renderLayers() {
        ...
        const layer = new ScatterplotLayer({
            ...
            getFillColor: (d: ScatterplotData) => this.projectColorMap.get(d.projectId) || [128, 128, 128],
