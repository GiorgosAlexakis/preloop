...
(function(global) {
  ...
  var cosineDistance = function(x1, x2) {
    var dot = dotProduct(x1, x2);
    var norm1 = norm(x1);
    var norm2 = norm(x2);
  ...
  var normalizeData = function(X) {
    var normalized = [];
    for(var i=0; i<X.length; i++) {
      var x = X[i];
      var n = norm(x);
      if (n === 0) {
        // Handle zero vectors by keeping them as zero
        normalized.push(x.slice()); // copy the zero vector
      } else {
        var normalizedX = [];
        for(var j=0; j<x.length; j++) {
          normalizedX.push(x[j] / n);
        }
        normalized.push(normalizedX);
      }
    }
    return normalized;
  }
  ...
  var xtod = function(X, distanceMetric) {
    var N = X.length;
    var dist = zeros(N * N); // allocate contiguous array
    var distFunc = distanceMetric === 'cosine' ? cosineDistance : L2;

    for(var i=0;i<N;i++) {
      for(var j=i+1;j<N;j++) {
        var d = distFunc(X[i], X[j]);
        dist[i*N+j] = d;
        dist[j*N+i] = d;
      }
    }
  ...
  var tSNE = function(opt) {
    ...
    this.metric = getopt(opt, "metric", "euclidean"); // distance metric: "euclidean" or "cosine"
  ...
  tSNE.prototype = {
    ...
    initDataRaw: function(X) {
      ...
      if (this.metric === "cosine") {
        processedX = normalizeData(X);
      }

      var dists = xtod(processedX, this.metric); // convert X to distances
