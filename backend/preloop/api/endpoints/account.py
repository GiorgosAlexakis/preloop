...
async def list_account_runtime_sessions(
    ...
    query: Optional[str] = Query(None, min_length=1),
    session_source_type: Optional[str] = Query(None),
    tool_name: Optional[str] = Query(None, min_length=1),
    status: str = Query("all", pattern="^(all|active|ended)$"),
    start_date: Optional[datetime] = Query(None),
    end_date: Optional[datetime] = Query(None),
    min_total_tokens: Optional[int] = Query(None, ge=0),
    max_total_tokens: Optional[int] = Query(None, ge=0),
    min_estimated_cost: Optional[float] = Query(None, ge=0),
    max_estimated_cost: Optional[float] = Query(None, ge=0),
    sort_by: str = Query("last_activity", pattern="^(last_activity|total_tokens|estimated_cost)$"),
