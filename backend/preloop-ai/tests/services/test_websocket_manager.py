...
from preloop_ai.services.websocket_manager import (
    WebSocketManager,
    nats_consumer,
...
class TestPersistExecutionLog:
    ...
    @patch("preloop_ai.services.websocket_manager.get_db")
    ...
    @patch("preloop_ai.services.websocket_manager.get_db")
    @patch("preloop_ai.services.websocket_manager.logger")
...
class TestNatsConsumer:
    ...
    @patch("preloop_ai.services.websocket_manager.get_task_publisher")
    @patch("preloop_ai.services.websocket_manager.persist_execution_log")
    ...
    async def test_nats_consumer_handles_invalid_json(self, mock_get_publisher):
        ...
        with patch("preloop_ai.services.websocket_manager.logger") as mock_logger:
            await captured_handler(mock_msg)
            assert mock_logger.warning.called
    ...
    async def test_nats_consumer_message_without_execution_id(self, mock_get_publisher):
        ...
        with patch(
            "preloop_ai.services.websocket_manager.persist_execution_log"
        ) as mock_persist:
            await captured_handler(mock_msg)
