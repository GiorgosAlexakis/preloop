...
from preloop_ai.services.mcp_http import (
    get_mcp_server,
...
class TestPreloopBearerAuthBackend:
    ...
    async def test_authenticate_invalid_token(self, mock_db, mock_account):
        ...
        with patch("preloop_ai.services.mcp_http.get_db") as mock_get_db:
            mock_get_db.return_value = iter([mock_db])
            with patch(
                "preloop_ai.services.mcp_http.get_user_from_token_if_valid",
                return_value=None,
            ):
                result = await backend.authenticate(conn)
...
class TestGetMCPServer:
    ...
    def test_get_mcp_server_creates_instance(self):
        ...
        # Reset global
        import preloop_ai.services.mcp_http as mcp_http_module

        mcp_http_module._mcp_server_instance = None

        with patch(
            "preloop_ai.services.mcp_http.initialize_dynamic_mcp_server"
        ) as mock_init:
...
class TestGetUserContextForMCP:
    ...
    async def test_get_user_context_no_cached_account(
        ...
        with patch(
            ...
            with patch("preloop_ai.services.mcp_http.has_tracker", return_value=True):
    ...
    async def test_get_user_context_cached_account(
        ...
        with patch("preloop_ai.services.mcp_http.has_tracker", return_value=False):
            context = await get_user_context_for_mcp(mock_request, mock_db)
...
class TestMCPHTTPStreamingEndpoint:
    ...
    async def test_endpoint_initialize_method(self, mock_request):
        ...
        with patch("preloop_ai.services.mcp_http.get_mcp_server"):
    ...
    async def test_endpoint_tools_list(self, mock_request):
        ...
        with patch(
            "preloop_ai.services.mcp_http.get_mcp_server", return_value=mock_server
        ):
            response = await mcp_http_streaming_endpoint(mock_request, user_context)
...
class TestSetupMCPRoutes:
    ...
    def test_setup_mcp_routes(self):
        ...
        with patch(
            "preloop_ai.services.initialize_mcp.initialize_mcp_with_tools",
            return_value=mock_mcp,
        ):
            with patch(
                "preloop_ai.services.dynamic_fastmcp_http.setup_dynamic_mcp_http",
