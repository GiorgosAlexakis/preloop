...
from preloop_ai.agents.container import ContainerAgentExecutor


@pytest.fixture
def mock_docker():
    """Mock aiodocker Docker client."""
    with patch("preloop_ai.agents.container.aiodocker.Docker") as mock:
...
class TestContainerAgentExecutor:
    ...
    async def test_start_kubernetes_fallback(self, mock_docker):
        ...
        with patch("preloop_ai.agents.container.KUBERNETES_AVAILABLE", False):
            executor = ContainerAgentExecutor(
                agent_type="test-agent",
                config={},
                image="test-image:latest",
    ...
    async def test_container_labels(self, container_executor, mock_docker):
        ...
        async def capture_create(config):
            ...
            assert labels["preloop_ai.agent_type"] == "test-agent"
            assert labels["preloop_ai.flow_id"] == "flow-123"
            assert labels["preloop_ai.execution_id"] == "exec-456"
