...
from preloop_ai.api.auth.router import authenticate_user
...
async def test_authenticate_user_updates_last_login(mock_user, mock_db_session):
    ...
    with (
        patch("preloop_ai.api.auth.router.get_db_session") as mock_get_db,
        patch("preloop_ai.api.auth.router.crud_user") as mock_crud_user,
        patch("preloop_ai.api.auth.router.verify_password") as mock_verify_password,
    ):
        # Setup mocks
