...
from preloop_ai.api.endpoints import webhooks
...
class TestWebhooksEndpoint:
    ...
    def setup_class(cls):
        ...
        cls.connect_nats_patcher = patch(
            "preloop_ai.api.app.connect_nats", new_callable=AsyncMock
        )
        cls.close_nats_patcher = patch(
            "preloop_ai.api.app.close_nats", new_callable=AsyncMock
        )
    ...
    @patch("preloop_ai.api.endpoints.webhooks.crud_organization")
    ...
    @patch("preloop_ai.api.endpoints.webhooks.crud_issue_embedding")
    @patch("preloop_ai.api.endpoints.webhooks.crud_issue")
    @patch("preloop_ai.api.endpoints.webhooks.crud_project")
    @patch("preloop_ai.api.endpoints.webhooks.TrackerClient")
    ...
    @patch(
        "preloop_ai.api.endpoints.webhooks.crud_organization"
    ...
    @patch("preloop_ai.api.endpoints.webhooks.get_task_publisher")
    @patch("preloop_ai.api.endpoints.webhooks.crud_project")
...
@pytest.mark.asyncio
@patch("preloop_models.db.session.get_db_session")
@patch("preloop_sync.services.event_bus.get_task_publisher")
