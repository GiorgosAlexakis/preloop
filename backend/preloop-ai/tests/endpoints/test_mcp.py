...
from preloop_ai.api.endpoints import mcp
from preloop_ai.api.endpoints.search import SearchResponse as ApiSearchResponse
from preloop_ai.schemas.issue import IssueResponse
from preloop_ai.schemas.mcp import (
    SuggestedUpdate,
    UpdateIssueRequest,
    GetIssueResponse,
)
...
async def test_mcp_get_issue_success(db_session: Session, test_user: User):
    ...
    with (
        patch("preloop_ai.api.endpoints.mcp.get_http_request") as mock_get_request,
        patch(
            "preloop_ai.api.endpoints.mcp.get_user_from_token_if_valid",
            new_callable=AsyncMock,
        ) as mock_get_user,
        patch("preloop_ai.api.endpoints.mcp.get_db") as mock_get_db,
    ):
...
async def test_mcp_create_issue_success(db_session: Session, test_user: User):
    ...
    with (
        ...
        patch(
            "preloop_ai.api.endpoints.mcp.api_create_issue", new_callable=AsyncMock
        ) as mock_api_create,
        patch("preloop_ai.api.endpoints.mcp.get_db") as mock_get_db,
    ):
        mock_get_request.return_value.headers = {"authorization": "Bearer testtoken"}
...
async def test_mcp_update_issue_success(db_session: Session, test_user: User):
    ...
    with (
        ...
        patch(
            "preloop_ai.api.endpoints.mcp.get_tracker_client", new_callable=AsyncMock
        ) as mock_get_tracker,
        patch("preloop_ai.api.endpoints.mcp.get_db") as mock_get_db,
    ):
        mock_get_request.return_value.headers = {"authorization": "Bearer testtoken"}
...
async def test_mcp_search_success(db_session: Session, test_user: User):
    ...
    with (
        ...
        patch(
            "preloop_ai.api.endpoints.search.perform_search", new_callable=AsyncMock
...
async def test_mcp_estimate_compliance_success(db_session: Session, test_user: User):
    ...
    with (
        patch("preloop_ai.api.endpoints.mcp.get_http_request"),
        patch(
            "preloop_ai.api.endpoints.mcp._get_authenticated_user",
            new_callable=AsyncMock,
        ) as mock_auth,
        patch(
            "preloop_ai.api.endpoints.mcp._process_single_issue_estimate",
            new_callable=AsyncMock,
        ) as mock_process,
    ):
...
async def test_mcp_improve_compliance_success(db_session: Session, test_user: User):
    ...
    with (
        ...
        patch(
            "preloop_ai.api.endpoints.mcp._process_single_issue_compliance",
            new_callable=AsyncMock,
        ) as mock_process,
    ):
        mock_auth.return_value = (db_session, test_user)
...
async def test_mcp_add_comment_success(db_session: Session, test_user: User):
    ...
    with (
        ...
        patch("preloop_ai.api.endpoints.mcp.get_db") as mock_get_db,
        patch(
            "preloop_ai.api.endpoints.mcp.get_tracker_client",
            new_callable=AsyncMock,
        ) as mock_get_tracker,
    ):
        mock_get_request.return_value.headers = {"authorization": "Bearer testtoken"}
