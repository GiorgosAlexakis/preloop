...
from preloop_ai.api.endpoints.issue_dependencies import (
    detect_issue_dependencies,
    DependencyRequest,
    DependencyResponse,
)
from preloop_models.models import Issue, AIModel, Project, Account
...
def test_detect_issue_dependencies_success(
    ...
    mock_crud_issue = mocker.patch(
        "preloop_ai.api.endpoints.issue_dependencies.crud_issue"
    )
    mock_crud_issue.get.side_effect = mock_issues

    mock_crud_ai_model = mocker.patch(
        "preloop_ai.api.endpoints.issue_dependencies.crud_ai_model"
    )
    ...
    mock_crud_issue_set = mocker.patch(
        "preloop_ai.api.endpoints.issue_dependencies.crud_issue_set"
    )
    mock_crud_issue_set.get_supersets_by_issues.return_value = []

    # Mock OpenAI client
    mock_openai_client = mocker.patch(
        "preloop_ai.api.endpoints.issue_dependencies.openai.OpenAI"
    )
    ...
    mocker.patch(
        "preloop_ai.api.endpoints.issue_dependencies.load_dependencies_prompts_config",
