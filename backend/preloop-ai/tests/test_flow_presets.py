...
from preloop_ai.flow_presets import (
    _extract_order,
    _load_yaml_file,
    load_flow_presets,
)
...
class TestLoadFlowPresets:
    ...
    def test_empty_presets_dir(self, tmp_path: Path):
        ...
        with patch("preloop_ai.flow_presets.PRESETS_DIR", presets_dir):
            # Clear the lru_cache
    ...
    def test_missing_presets_dir(self, tmp_path: Path):
        ...
        with patch("preloop_ai.flow_presets.PRESETS_DIR", nonexistent_dir):
            load_flow_presets.cache_clear()
            result = load_flow_presets()
            assert result == []
