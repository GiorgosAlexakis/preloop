...
def client():
    ...
    with (
        patch("preloop_ai.api.app.connect_nats", new_callable=AsyncMock),
        patch("preloop_ai.api.app.close_nats", new_callable=AsyncMock),
        patch(
            "preloop_ai.services.websocket_manager.nats_consumer",
...
def test_pyinstrument_middleware_disabled_by_default(client):
    ...
    """
    with patch("preloop_ai.api.app.Profiler") as mock_profiler:
...
def test_pyinstrument_middleware_enabled(client):
    ...
    """
    with (
        patch("preloop_ai.api.app.Profiler") as mock_profiler_cls,
        patch("preloop_ai.api.app.SpeedscopeRenderer") as mock_renderer_cls,
    ):
...
def test_api_usage_middleware_excluded_route(client):
    ...
    """
    with patch("preloop_ai.api.app.get_db_session") as mock_get_db_session:
        response = client.get("/api/v1/health")
        assert response.status_code == 200
...
@patch("preloop_ai.api.auth.jwt.decode_token")
def test_api_usage_middleware_db_error(mock_decode_token, client):
    ...
    with patch("preloop_ai.api.app.get_db_session", return_value=mock_db_gen):
...
@patch("preloop_ai.api.app.init_sentry")
@patch("preloop_ai.api.app.setup_database")
...
@patch("preloop_ai.api.app.setup_database", side_effect=Exception("DB setup failed"))
...
@patch(
    "preloop_ai.api.app.connect_nats",
    new_callable=AsyncMock,
