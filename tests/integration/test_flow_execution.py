...
from spacemodels.models import Flow, AIModel, Account, FlowExecution
...
class TestFlowExecution:
    ...
    async def test_container_inspection(self, db: Session, docker_client):
        ...
        # List all containers with spacebridge labels
        containers = await docker_client.containers.list(
            filters={"label": "spacebridge.agent_type"}
        )

        logger.info(f"Found {len(containers)} SpaceBridge agent containers")
        ...
        for container in containers:
            ...
            flow_id = labels.get("spacebridge.flow_id")
            execution_id = labels.get("spacebridge.execution_id")
            agent_type = labels.get("spacebridge.agent_type")
    ...
    async def test_mcp_configuration(self, test_flow: Flow):
        ...
        # Check that spacebridge-mcp is configured
        assert "spacebridge-mcp" in mcp_config["mcpServers"]
        assert "spacebridge-mcp" in mcp_config["allowed_tools"]
        assert "search_issues" in mcp_config["allowed_tools"]["spacebridge-mcp"]
        ...
        assert "spacebridge-mcp" in mcp_env["MCP_ALLOWED_SERVERS"]
