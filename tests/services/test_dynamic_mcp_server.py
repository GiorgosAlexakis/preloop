...
class TestHelperFunctions:
    ...
    def test_get_dynamic_mcp_server_creates_singleton(self):
        ...
        import spacebridge.services.dynamic_mcp_server as module
    ...
    def test_register_default_tools(self):
        ...
        with patch("spacebridge.api.endpoints.mcp.get_issue", AsyncMock()):
            with patch("spacebridge.api.endpoints.mcp.create_issue", AsyncMock()):
                with patch("spacebridge.api.endpoints.mcp.update_issue", AsyncMock()):
                    with patch("spacebridge.api.endpoints.mcp.search", AsyncMock()):
                        with patch(
                            "spacebridge.api.endpoints.mcp.estimate_compliance",
                            AsyncMock(),
                        ):
                            with patch(
                                "spacebridge.api.endpoints.mcp.improve_compliance",
                                AsyncMock(),
                            ):
