...
import { getTools, getApprovalWorkflows, deleteApprovalWorkflow, createToolConfiguration, updateToolConfiguration, getFeatures, fetchWithAuth, } from '../../api';
...
let PoliciesView = class PoliciesView extends LitElement {
    constructor() {
        super(...arguments);
        this._activeTab = 'access';
        this._tools = [];
        ...
        this._expandedVersions = new Set();
    ...
    async handleAccessActionChange(rule, newAction) {
        try {
            const tool = this._tools.find((t) => t.name === rule.toolName &&
            ...
            if (newAction === 'deny') {
                ...
                const defaultPolicy = this._approvalPolicies.find((p) => p.is_default) ||
    ...
    async deletePolicy(policy) {
        if (!confirm(`Are you sure you want to delete the policy "${policy.name}"? This cannot be undone.`)) {
            return;
        }
        try {
    ...
    async loadVersions() {
        ...
        try {
            const response = await fetchWithAuth('/api/v1/policies/versions?limit=50');
            if (!response.ok) {
    ...
    async createVersion() {
        ...
        try {
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail?.message || error.detail || 'Failed to save version');
            }
            this._showSaveVersionDialog = false;
    ...
    async rollbackToVersion(versionId, previewOnly) {
        if (previewOnly) {
            this._rollingBack = true;
        }
        try {
            const response = await fetchWithAuth(`/api/v1/policies/versions/${versionId}/rollback`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail?.message || error.detail || 'Failed to rollback');
            }
    ...
    async tagVersion(versionId, tag) {
        ...
        try {
            const response = await fetchWithAuth(`/api/v1/policies/versions/${versionId}/tag`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail?.message || error.detail || 'Failed to update tag');
            }
            this._showTagDialog = false;
            this._versionToTag = null;
            this._tagForm = { tag: '' };
    ...
    async deleteVersion(version) {
        if (!confirm(`Are you sure you want to delete version ${version.version_number}${version.tag ? ` (${version.tag})` : ''}? This cannot be undone.`)) {
            return;
        }
        this._deletingVersion = true;
        try {
            const response = await fetchWithAuth(`/api/v1/policies/versions/${version.id}`, { method: 'DELETE' });
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail?.message || error.detail || 'Failed to delete version');
            }
            await this.loadVersions();
        }
        catch (err) {
    ...
    async pruneVersions() {
        ...
        try {
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail?.message || error.detail || 'Failed to prune versions');
            }
            const result = await response.json();
            this._showPruneDialog = false;
    ...
    renderAccessPoliciesTab() {
      ...
      <div class="access-rules-list">
        ${sortedRules.length === 0
            ...
            `
            : repeat(sortedRules, (rule) => this.getToolKey(rule), (rule) => this.renderAccessRuleCard(rule))}
      </div>
    `;
    }
    renderAccessRuleCard(rule) {
        const toolKey = this.getToolKey(rule);
        const isExpanded = this._expandedTools.has(toolKey);
        const assignedPolicy = this._approvalPolicies.find((p) => p.id === rule.workflowId);
        return html `
        ...
        >
          ...
          >
            ...
            <sl-select
              size="small"
              value=${rule.action}
              @sl-change=${(e) => this.handleAccessActionChange(rule, e.target.value)}
      ...
      ${this._approvalPolicies.length === 0
          ...
          `
            ...
            <div class="policies-grid">
              ${repeat(this._approvalPolicies, (policy) => policy.id, (policy) => this.renderPolicyCard(policy))}
            </div>
          `}
          ...
          <div class="policy-meta">
            ...
            ? html `
                  ...
                  <div class="policy-meta-item">
                    ...
                    <span>
                      ${Math.round((policy.ai_confidence_threshold || 0.8) * 100)}%
                      threshold
                    </span>
                  </div>
                  <div class="policy-meta-item">
    ...
    renderPolicyFilesTab() {
      ...
      <div class="policy-files-container">
        <div class="policy-files-actions">
          <sl-button
            variant="primary"
            @click=${() => this.shadowRoot
            ?.querySelector('#policy-file-input')
            ?.click()}
            ...
            ? html `
              <sl-card>
                ...
                <div class="history-list">
                  ${repeat(this._policyFileHistory, (item) => item.id, (item) => html `
                      <div
            ...
            : this._versions.length === 0
              ...
              `
                ...
                <div class="version-list">
                  ${repeat(this._versions, (v) => v.id, (version) => this.renderVersionItem(version))}
                </div>
              `}
      </div>
    `;
            ...
            ? html `
              ...
              <div class="form-field">
                ...
                <sl-input
                  ...
                  @sl-input=${(e) => (this._tagForm = { tag: e.target.value })}
                ></sl-input>
              </div>
              ...
              <div slot="footer" class="dialog-footer">
                ...
                <sl-button
                  variant="primary"
                  @click=${() => this.tagVersion(this._versionToTag.id, this._tagForm.tag)}
        ...
        ${this._versionToRollback
            ? html `
                    ...
                    ? html `
                        ...
                        ? html `
                            ...
                            .added.length})
                                    ...
                                    ${this._rollbackPreview.changes.changes.added.map((change) => html `
                        ...
                        ? html `
                            ...
                            .modified.length})
                                    ...
                                    ${this._rollbackPreview.changes.changes.modified.map((change) => html `
                        ...
                        ? html `
                            ...
                            .removed.length})
                                    ...
                                    ${this._rollbackPreview.changes.changes.removed.map((change) => html `
              ...
              <div slot="footer" class="dialog-footer">
                ...
                <sl-button
                  variant="danger"
                  @click=${() => this.rollbackToVersion(this._versionToRollback.id, false)}
                ...
                ? html `
                    ...
                    ? html `
                            <div class="diff-section">
                              ...
                              ${this._diffResult.changes.added.map((change) => html `
                                  <div class="diff-item added">
                                    <strong>${change.category}:</strong>
                                    ${change.name}
                                    ${change.details
                    ...
                    ? html `
                            <div class="diff-section">
                              ...
                              ${this._diffResult.changes.modified.map((change) => html `
                                  <div class="diff-item modified">
                                    <strong>${change.category}:</strong>
                                    ${change.name}
                                    ${change.details
                    ...
                    ? html `
                            <div class="diff-section">
                              ...
                              ${this._diffResult.changes.removed.map((change) => html `
                                  <div class="diff-item removed">
                                    <strong>${change.category}:</strong>
                                    ${change.name}
                                  </div>
      ...
      <div class="column-layout extra-wide">
        <div class="main-column">
          ...
          ${this._loading
            ...
            : html `
                <sl-tab-group
                  @sl-tab-show=${(e) => (this._activeTab = e.detail.name)}
                >
                  <sl-tab
                    slot="nav"
                    panel="access"
...
__decorate([
    state()
], PoliciesView.prototype, "_activeTab", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_tools", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_approvalPolicies", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_loading", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_error", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_features", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_toolAccessRules", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_expandedTools", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showPolicyDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_editingPolicy", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_policyFileHistory", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showDiffDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_diffResult", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_pendingFile", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_isUploading", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_isExporting", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_versions", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_loadingVersions", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_selectedVersion", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_expandedVersions", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showSaveVersionDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showPruneDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showTagDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_showRollbackDialog", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_rollbackPreview", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_savingVersion", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_pruningVersions", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_rollingBack", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_taggingVersion", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_deletingVersion", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_versionForm", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_pruneForm", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_tagForm", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_versionToTag", void 0);
__decorate([
    state()
], PoliciesView.prototype, "_versionToRollback", void 0);
PoliciesView = __decorate([
