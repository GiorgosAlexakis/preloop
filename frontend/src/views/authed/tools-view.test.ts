...
describe('ToolsView (approvals + conditions)', () => {
  ...
  beforeEach(() => {
    ...
    fetchStub.callsFake(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/api/v1/tool-configurations/cfg-1/condition') && method === 'PUT') {
  ...
  it('does not create tool configuration twice when adding a condition immediately after enabling approvals', async () => {
    const element = (await fixture(html`<tools-view></tools-view>`)) as ToolsView;
    ...
    const toolCard = element.shadowRoot?.querySelector('tool-card') as HTMLElement;
    ...
    const toolConfigCreateCalls = fetchStub
      .getCalls()
      .filter((c) => {
        const url = String(c.args[0]);
        const method = String((c.args[1] as RequestInit | undefined)?.method || 'GET').toUpperCase();
