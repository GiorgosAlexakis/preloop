...
describe('RuntimeSessionsView', () => {
    ...
    beforeEach(() => {
        ...
        fetchStub.callsFake(async (input) => {
            ...
            if (url.startsWith('/api/v1/runtime-sessions/runtime-session-1')) {
                if (String(input.method || 'GET').toUpperCase() === 'PATCH') {
                    return new Response(JSON.stringify({
                        id: 'runtime-session-1',
                        session_source_type: 'claude_code',
                        session_source_id: 'workspace-42',
                        ...
                        items: [
                            {
                                ...
                                excerpt: 'request.input: Summarize the deployment risk review response.output_text: Deployment risk review summarized',
                                meta_data: {
            ...
            if (url.startsWith('/api/v1/flows/executions/execution-1/gateway-events')) {
                return new Response(JSON.stringify({
                    source: 'database',
                    logs: [
                        {
    ...
    it('renders runtime session list and selected session detail', async () => {
        const element = (await fixture(html `<runtime-sessions-view></runtime-sessions-view>`));
        await waitUntil(() => !element.loading && !element.detailLoading, 'Runtime sessions view did not finish loading');
        ...
        const listCall = fetchStub
            .getCalls()
            .find((call) => String(call.args[0]).startsWith('/api/v1/runtime-sessions?'));
        const detailCall = fetchStub
            .getCalls()
            .find((call) => String(call.args[0]).startsWith('/api/v1/runtime-sessions/runtime-session-1'));
    ...
    it('shows flow-backed session content from execution gateway events', async () => {
        const element = (await fixture(html `<runtime-sessions-view></runtime-sessions-view>`));
        await waitUntil(() => !element.loading && !element.detailLoading, 'Runtime sessions view did not finish loading');
        const sessionButtons = element.shadowRoot?.querySelectorAll('.session-item');
        (sessionButtons?.[1]).click();
        await waitUntil(() => element.detail?.session?.id === 'runtime-session-2' &&
            !element.gatewayEventsLoading, 'Flow-backed session detail did not finish loading');
        await element.updateComplete;
        const content = (element.shadowRoot?.textContent || '').replace(/\s+/g, ' ');
        ...
        const gatewayEventsCall = fetchStub
            .getCalls()
            .find((call) => String(call.args[0]).startsWith('/api/v1/flows/executions/execution-1/gateway-events'));
