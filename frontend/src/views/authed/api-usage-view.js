...
import { getAccountGatewayUsageSearch, getAccountGatewayUsageSummary, } from '../../api';
import consoleStyles from '../../styles/console-styles.css?inline';
let ApiUsageView = class ApiUsageView extends LitElement {
    constructor() {
        super(...arguments);
        this.summary = null;
    ...
    getSessionSourceType(session) {
        return (session.session_source_type ??
            (session.flow_execution_id ? 'flow_execution' : null));
    }
    getSessionSourceId(session) {
        return (session.session_source_id ??
    ...
    getSessionSourceLabel(sourceType) {
        if (!sourceType) {
    ...
    getSessionDisplayName(session) {
        return (session.runtime_session_name ??
    ...
    isFlowBackedSession(session) {
        return (this.getSessionSourceType(session) === 'flow_execution' &&
            Boolean(this.getSessionSourceId(session)));
    }
    getRuntimeSessionHref(session) {
    ...
    renderStatCard(label, value, detail, icon) {
        return html `
      <div class="stat-card">
    ...
    renderModelBreakdown(models) {
      ...
      `;
        ...
        const sortedModels = [...models].sort((left, right) => right.estimated_cost - left.estimated_cost ||
    ...
    renderFlowBreakdown(flows) {
      ...
      `;
        ...
        const sortedFlows = [...flows].sort((left, right) => right.request_count - left.request_count ||
    ...
    renderSessionBreakdown(sessions) {
      ...
      `;
        ...
        const sortedSessions = [...sessions].sort((left, right) => new Date(this.getSessionLastActivity(right) || 0).getTime() -
    ...
    renderSummary(summary) {
      ...
      <div class="stats-grid">
        ${this.renderStatCard('Requests', this.formatNumber(summary.total_requests), `${this.formatNumber(summary.successful_requests)} succeeded, ${this.formatNumber(summary.failed_requests)} failed`, 'activity')}
        ${this.renderStatCard('Estimated Cost', this.formatCost(summary.estimated_cost), `${this.formatCost(summary.budget.current_spend_usd)} current spend`, 'cash')}
        ${this.renderStatCard('Total Tokens', this.formatNumber(tokenUsage.total_tokens), `${this.formatNumber(tokenUsage.prompt_tokens)} prompt, ${this.formatNumber(tokenUsage.completion_tokens)} completion`, 'cpu')}
        ${this.renderStatCard('Success Rate', this.formatPercent(successRate), `${this.formatDateTimeLabel(summary.period_start)} to ${this.formatDateTimeLabel(summary.period_end)}`, 'check2-circle')}
      </div>
    `;
    }
    render() {
            ...
            : this.summary
                ? html `
                    ...
                    <sl-card class="breakdown-card">
                      ...
                      ${this.renderSessionBreakdown(this.summary.usage_by_session)}
                    </sl-card>
                    ...
                    <div class="content-grid">
                      <div class="stack">
                        ...
                        <sl-card class="breakdown-card">
                          ...
                          ${this.renderModelBreakdown(this.summary.usage_by_model)}
                        </sl-card>
                      </div>
                      ...
                      <div class="stack">
                        ...
                        <sl-card class="breakdown-card">
                          ...
                          ${this.renderFlowBreakdown(this.summary.usage_by_flow)}
                        </sl-card>
                      </div>
                    </div>
                  `
...
__decorate([
    state()
], ApiUsageView.prototype, "summary", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "searchResults", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "loading", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "error", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "selectedRange", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "startDate", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "endDate", void 0);
__decorate([
    state()
], ApiUsageView.prototype, "searchQuery", void 0);
ApiUsageView = __decorate([
