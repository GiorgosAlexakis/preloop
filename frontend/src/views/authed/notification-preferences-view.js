...
let NotificationPreferencesView = class NotificationPreferencesView extends AuthedElement {
    constructor() {
        super(...arguments);
    ...
    async connectedCallback() {
        ...
        try {
            console.debug('[NotificationPrefs] Setting up device_registered subscription');
            this.unsubscribe = unifiedWebSocketManager.subscribe('device_registered', (message) => {
                console.debug('[NotificationPrefs] Received device_registered event:', message);
    ...
    async handleToggleMobilePush(e) {
        if (!this.preferences)
            return;
        try {
            this.isSaving = true;
    ...
    async handleShowQRCode() {
        try {
            const data = await this.fetchData('/api/v1/notification-preferences/me/qr-code');
            if (!data) {
    ...
    async handleUnregisterDevice(token) {
        if (!confirm('Are you sure you want to unregister this device?')) {
            return;
        }
        try {
            const data = await this.fetchData(`/api/v1/notification-preferences/me/device/${encodeURIComponent(token)}`, {
                method: 'DELETE',
            });
            if (!data) {
          ...
          ${this.preferences?.mobile_device_tokens &&
            ...
            ? html `
                <div class="devices-list">
                  ${this.preferences.mobile_device_tokens.map((device) => html `
                ...
                : 'success'}
                        ...
                        <sl-button
                          size="small"
                          variant="danger"
                          @click=${() => this.handleUnregisterDevice(device.token)}
                        >
...
NotificationPreferencesView.styles = css `
    :host {
      display: block;
...
__decorate([
    state()
], NotificationPreferencesView.prototype, "preferences", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "isLoading", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "isSaving", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "errorMessage", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "successMessage", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "showQRDialog", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "qrCodeData", void 0);
__decorate([
    state()
], NotificationPreferencesView.prototype, "qrExpiry", void 0);
NotificationPreferencesView = __decorate([
