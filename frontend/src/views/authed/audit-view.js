...
let AuditView = class AuditView extends AuthedElement {
    constructor() {
        super(...arguments);
        // Timeline data
        this._groups = [];
    ...
    async _loadUsers() {
        try {
            ...
            if (res.ok) {
                ...
                this._userMap = new Map(this._users.map((u) => [u.id, u]));
            }
        }
        catch (e) {
    ...
    async _loadTimeline() {
        ...
        try {
            ...
            if (this._toolNameFilter)
                params.set('tool_name', this._toolNameFilter);
            if (this._startDate)
    ...
    _formatActorLabel(event) {
        const actor = 'user_id' in event ? this._getUserDisplay(event.user_id) : 'System';
    ...
    _prettyDetailLabel(key) {
        return key
            .replace(/_/g, ' ')
            .replace(/\b\w/g, (char) => char.toUpperCase());
    }
    _renderDetailItem(label, value) {
    ...
    _renderEventDetails(details) {
        if (!details || !this._hasExpandableDetails(details))
            return nothing;
        const preferredKeys = [
            'api_key_name',
            'api_key_id',
        ...
        const preferredItems = preferredKeys
            ...
            return this._renderDetailItem(this._prettyDetailLabel(key), details[key]);
        });
        const remainingItems = Object.entries(details)
            .filter(([key, value]) => !renderedKeys.has(key) &&
            value != null &&
            typeof value !== 'object' &&
            !['tool_name', 'decision', 'event'].includes(key))
            .map(([key, value]) => this._renderDetailItem(this._prettyDetailLabel(key), value));
        return html `
    ...
    _getActionIcon(action) {
        if (action === 'tool_call')
            return 'terminal';
        if (action === 'model_gateway_request')
            return 'cpu';
        ...
        if (action === 'permission_check')
            return 'lock';
        if (action.startsWith('runtime_session_'))
            return 'activity';
        if (action.startsWith('role_'))
            return 'people';
        return 'info-circle';
    }
    _getSubEventLabel(sub) {
        const d = sub.details || {};
        switch (sub.action) {
            case 'policy_allow':
    ...
    _getPrimaryLabel(event) {
        switch (event.action) {
            ...
            case 'model_gateway_request': {
                const modelLabel = event.details?.requested_model ||
                    event.details?.model_alias ||
                    event.resource_id ||
                    'request';
                const providerLabel = event.details?.gateway_provider ||
    ...
    _getArgsSummary(details) {
        if (!details?.tool_args)
            return '';
    ...
    _canExpandGroup(group) {
        return (group.sub_events.length > 0 ||
    ...
    _renderFilterBar() {
      ...
      <div class="filter-bar">
        <sl-input
          ...
          @sl-input=${(e) => {
            this._toolNameFilter = e.target.value;
        }}
          @sl-clear=${() => {
            this._toolNameFilter = '';
            this._applyFilters();
        ...
        >
          ${EVENT_TYPE_OPTIONS.map((opt) => html `
    ...
    _renderGroup(group) {
        ...
        const execTime = event.details?.execution_time_ms ?? event.details?.duration_ms;
        return html `
      <div
...
__decorate([
    state()
], AuditView.prototype, "_groups", void 0);
__decorate([
    state()
], AuditView.prototype, "_loading", void 0);
__decorate([
    state()
], AuditView.prototype, "_total", void 0);
__decorate([
    state()
], AuditView.prototype, "_page", void 0);
__decorate([
    state()
], AuditView.prototype, "_pageSize", void 0);
__decorate([
    state()
], AuditView.prototype, "_eventTypeFilters", void 0);
__decorate([
    state()
], AuditView.prototype, "_outcomeFilters", void 0);
__decorate([
    state()
], AuditView.prototype, "_toolNameFilter", void 0);
__decorate([
    state()
], AuditView.prototype, "_startDate", void 0);
__decorate([
    state()
], AuditView.prototype, "_endDate", void 0);
__decorate([
    state()
], AuditView.prototype, "_expandedGroups", void 0);
__decorate([
    state()
], AuditView.prototype, "_users", void 0);
AuditView = __decorate([
