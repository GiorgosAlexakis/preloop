...
describe('FlowsView', () => {
    let fetchStub;
    function createFetchStub(flows = [], presets = []) {
        return sinon
            .stub(window, 'fetch')
    ...
    it('renders the flow list view', async () => {
        ...
        const element = (await fixture(html `<flows-view></flows-view>`));
        await waitUntil(() => !element.isLoading, 'Flows view did not finish loading');
    ...
    it('shows flow cards when flows exist', async () => {
        ...
        const element = (await fixture(html `<flows-view></flows-view>`));
        await waitUntil(() => element.flows?.length === 1, 'Flows did not load');
        await element.updateComplete;
