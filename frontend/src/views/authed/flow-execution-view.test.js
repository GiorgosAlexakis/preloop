...
describe('FlowExecutionView', () => {
    ...
    beforeEach(() => {
        ...
        fetchStub.callsFake(async (input, init) => {
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-1/gateway-events') &&
                method === 'GET') {
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-1/logs') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    logs: [],
                    source: 'database',
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-1/metrics') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    tool_calls: 0,
                    api_requests: 1,
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-1') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    id: 'exec-1',
                    flow_id: 'flow-1',
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-running/gateway-events') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    logs: [],
                    source: 'database',
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-running/logs') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    logs: [],
                    source: 'database',
            ...
            if (url.endsWith('/api/v1/flows/executions/exec-running') &&
                method === 'GET') {
                return new Response(JSON.stringify({
                    id: 'exec-running',
    ...
    it('renders execution-scoped gateway events with payload details', async () => {
        const element = (await fixture(html `<flow-execution-view></flow-execution-view>`));
        element.executionId = 'exec-1';
        await element.updateComplete;
        await waitUntil(() => element.execution?.id === 'exec-1' &&
            element.gatewayEvents?.length === 1 &&
            !element.isLoading, 'Execution view did not finish loading');
        await element.updateComplete;
        const content = (element.shadowRoot?.textContent || '').replace(/\s+/g, ' ');
        ...
        const gatewayEventsCalls = fetchStub
            .getCalls()
            .filter((call) => String(call.args[0]).endsWith('/api/v1/flows/executions/exec-1/gateway-events'));
    ...
    it('updates execution metrics from live gateway events', async () => {
        ...
        await waitUntil(() => element.execution?.id === 'exec-1' &&
            element.gatewayEvents?.length === 1 &&
            !element.isLoading, 'Execution view did not finish loading');
        element.handleWebSocketMessage({
            execution_id: 'exec-1',
        ...
        await element.updateComplete;
        expect(element.gatewayEvents).to.have.length(2);
        expect(element.totalTokens).to.equal(5555);
        expect(element.budgetUsed).to.equal(0.345);
        expect(element.hasPricing).to.equal(true);
    });
    it('hydrates tool call metrics from the execution record on reload', async () => {
        const element = (await fixture(html `<flow-execution-view></flow-execution-view>`));
        element.executionId = 'exec-running';
        await element.updateComplete;
        await waitUntil(() => element.execution?.id === 'exec-running' &&
            !element.isLoading, 'Running execution view did not finish loading');
        expect(element.toolCalls).to.equal(3);
        expect(element.logs.some((log) => log.type === 'mcp_call')).to.equal(true);
        const content = (element.shadowRoot?.textContent || '').replace(/\s+/g, ' ');
