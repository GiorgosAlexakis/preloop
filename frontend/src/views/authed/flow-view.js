...
import { getFlow, createFlow, updateFlow, getTrackers, getAIModels, getFlowPresets, listOrganizations, listProjects, getAllTools, getMCPServers, } from '../../api';
import { unifiedWebSocketManager } from '../../services/unified-websocket-manager';
import { parseUTCDate, formatLocalDateTime, calculateDuration, } from '../../utils/date';
...
let FlowView = class FlowView extends LitElement {
    constructor() {
        super(...arguments);
        this.flow = {
            name: '',
    ...
    async connectedCallback() {
        ...
        if (this.flowId) {
            ...
            console.log('DEBUG: Loaded flow git_clone_config:', this.flow.git_clone_config);
            // Set trigger type based on flow
            this.triggerType =
                this.flow.trigger_event_source === 'webhook' ? 'webhook' : 'tracker';
            // Load recent executions for this flow
            const allExecutions = await import('../../api').then((m) => m.getFlowExecutions());
            this.recentExecutions = allExecutions
                .filter((exec) => exec.flow_id === this.flowId)
                .sort((a, b) => parseUTCDate(b.start_time).getTime() -
                parseUTCDate(a.start_time).getTime())
                .slice(0, 10);
            // Load all necessary data for editing (in parallel)
            this._loadingReferenceData = true;
            try {
                const [trackers, models, tools, servers, allOrganizations, allProjects,] = await Promise.all([
                    getTrackers(),
                    getAIModels(),
                    getAllTools(),
                    getMCPServers(),
            ...
            if (this.triggerType === 'tracker' && this.flow.trigger_event_source) {
                ...
                const trackerOrgs = (this.organizations || []).filter((org) => org.tracker_id === this.flow.trigger_event_source);
                ...
                if (this.flow.trigger_organization_id) {
                    const orgProjects = (this.projects || []).filter((proj) => proj.organization_id === this.flow.trigger_organization_id);
    ...
    connectToFlowUpdates() {
        // Connect to WebSocket for real-time flow execution updates
        this.unsubscribe = unifiedWebSocketManager.subscribe('flow_executions', (message) => {
            ...
            if (message.type === 'status_update' && message.execution_id) {
                const executionIndex = this.recentExecutions.findIndex((exec) => exec.id === message.execution_id);
                if (executionIndex !== -1) {
    ...
    renderFlowDetails() {
      ...
      >
        ...
        ${Object.keys(this.testRunPlaceholders).map((placeholder) => html `
            <sl-input
              label="${placeholder}"
              placeholder="Enter test value"
              .value=${this.testRunPlaceholders[placeholder]}
              @sl-input=${(e) => this.updatePlaceholderValue(placeholder, e.target.value)}
        ...
        <div class="main-column">
          ...
          <sl-card>
            ...
            ? html `
                    ...
                    <span
                      >${this.getOrganizationName(this.flow.trigger_organization_id)}</span
                    >
                  `
            : ''}
              ${this.flow.trigger_project_ids?.length
          ...
          ${this.flow.git_clone_config?.enabled &&
            ...
            ? html `
                <sl-card>
                  ...
                  ${(this.flow.git_clone_config.repositories || []).map((repo, index) => html `
                      <div
                ...
                : '0'};"
                      >
                        ...
                        >
                          ...
                          <span
                            >${this.trackers.find((t) => t.id === repo.tracker_id)?.name || repo.tracker_id}</span
                          >
...
${(this.flow.custom_commands.commands || []).join('\n')}</pre
          ...
          <sl-card>
            ...
            : html `
                  <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                      ${this.recentExecutions.map((exec) => html `
                          <tr>
    ...
    extractTriggerEventPlaceholders() {
        if (!this.flow.prompt_template)
            return [];
    ...
    async executeTestRun(triggerEventData) {
        if (!this.flowId)
            return;
        try {
            const execution = await import('../../api').then((m) => m.triggerFlowExecution(this.flowId, triggerEventData));
    ...
    renderCreationModeSelector() {
      ...
      <div class="creation-mode-toggle">
        ...
        <sl-radio-group
          ...
          @sl-change=${(event) => (this.creationMode = event.target.value)}
        >
    ...
    selectPreset(preset) {
        ...
        if (preset.allowed_mcp_tools && Array.isArray(preset.allowed_mcp_tools)) {
            this.flow.allowed_mcp_tools = preset.allowed_mcp_tools.map((tool) => ({
    ...
    async _autoPopulatePresetFields() {
        ...
        if (this.trackers.length > 0 && !this.flow.trigger_event_source) {
            ...
            this.organizations = allOrganizations.filter((org) => org.tracker_id === tracker.id);
            if (this.organizations.length > 0) {
                ...
                this.projects = allProjects;
                const orgProjects = allProjects.filter((proj) => proj.organization_id === org.id);
                if (orgProjects.length > 0) {
                    this.flow.trigger_project_ids = orgProjects.map((proj) => proj.id);
                }
            }
            else {
    ...
    getCompatibleModels() {
        ...
        return this.models.filter((model) => compatibleProviders.includes(model.provider_name?.toLowerCase()));
    }
    renderForm() {
        return html `
      <form @submit=${this.handleSubmit}>
        <sl-card>
          <sl-input
            ...
            @sl-input=${(e) => this.handleInputChange('name', e)}
            required
          ></sl-input>
          <sl-textarea
            label="Description"
            .value=${this.flow.description || ''}
            @sl-input=${(e) => this.handleInputChange('description', e)}
          ></sl-textarea>
        </sl-card>
        ...
        <sl-card>
          ...
          <div style="margin-bottom: 1.5rem;">
            ...
            <sl-radio-group
              ...
              @sl-change=${(e) => this.handleTriggerTypeChange(e.target.value)}
        ...
        <sl-card>
                ...
                : (() => {
                    ...
                    <div>
                      <sl-select
                        ...
                        @sl-change=${(e) => (this.flow.ai_model_id = e.target.value)}
                      ...
                      >
                        ...
                        : compatibleModels.map((model) => html `<sl-option value=${model.id}
          ...
          <sl-textarea
            ...
            @sl-input=${(e) => this.handleInputChange('prompt_template', e)}
            ...
            ? html `
              <sl-card>
                ...
                <sl-checkbox
                  ...
                  @sl-change=${(e) => this.handleGitCloneToggle(e.target.checked)}
                ...
                ? html `
                    ...
                    this.requestUpdate();
                          ...
                          >${this.getGitTrackers().some((t) => t.tracker_type === 'gitlab')
                    ...
                    ? html `
                              <div style="margin-top: 0.5rem;">
                                <sl-input
                                  label="${this.getGitTrackers().some((t) => t.tracker_type === 'gitlab')
                        ? 'MR Title (optional)'
                        : 'PR Title (optional)'}"
                                  .value=${this.flow.git_clone_config
                        ?.pull_request_title || ''}
                        ...
                        this.flow = {
                            ...
                            git_clone_config: {
                                ...this.flow.git_clone_config,
                                pull_request_description: e.target.value,
                            },
                        };
                    }}
                                  rows="3"
                ...
                ? html `
                      <div style="margin-top: 1rem;">
                        ...
                        <sl-textarea
                          ...
                          rows="5"
                          .value=${(this.flow.custom_commands.commands || []).join('\n')}
                          @sl-input=${(e) => {
                    if (this.flow.custom_commands) {
        ...
        <!-- <sl-card>
          ...
          <div class="form-grid">
            <sl-input
              ...
              @sl-input=${(e) => this.handleInputChange('max_iterations', e)}
            ></sl-input>
            <sl-input
              label="Max Budget"
              type="number"
              .value=${String(this.flow.max_budget || '')}
              @sl-input=${(e) => this.handleInputChange('max_budget', e)}
            ></sl-input>
          </div>
        </sl-card> -->
        ...
        <div>
          <sl-checkbox
            ...
            @sl-change=${(e) => (this.flow.is_preset = e.target.checked)}
    ...
    startPollingOrganizations(trackerId) {
        ...
        this.organizationPollingInterval = window.setInterval(async () => {
            const allOrganizations = await listOrganizations();
            const orgs = allOrganizations.filter((org) => org.tracker_id === trackerId);
            if (orgs.length > 0) {
    ...
    startPollingProjects(orgId) {
        ...
        this.projectPollingInterval = window.setInterval(async () => {
            const allProjects = await listProjects();
            const orgProjects = allProjects.filter((proj) => proj.organization_id === orgId);
            if (orgProjects.length > 0) {
    ...
    async handleTrackerChange(e) {
        ...
        const allOrganizations = await listOrganizations();
        this.organizations = allOrganizations.filter((org) => org.tracker_id === trackerId);
    ...
    getEventOptions() {
        const tracker = this.trackers.find((t) => t.id === this.flow.trigger_event_source);
        if (tracker) {
    ...
    handleEventChange(e) {
        const values = Array.from(e.target.value || []);
    ...
    renderToolSelection() {
      ...
      `;
        ...
        const builtinTools = this.availableTools.filter((tool) => tool.source === 'builtin');
        const supportedBuiltinTools = builtinTools.filter((tool) => tool.is_supported !== false);
        const unsupportedBuiltinTools = builtinTools.filter((tool) => tool.is_supported === false);
        const mcpTools = this.availableTools.filter((tool) => tool.source === 'mcp');
        return html `
      <div>
        ${builtinTools.length > 0
            ? html `
              <div style="margin-bottom: 1.5rem;">
                ...
                >
                  ${supportedBuiltinTools.map((tool) => html `
                      <sl-checkbox
                        .checked=${this.isToolSelected('preloop-mcp', tool.name)}
                        @sl-change=${(e) => this.handleToolToggle('preloop-mcp', tool.name, e.target.checked)}
    ...
    isToolSelected(serverName, toolName) {
        if (!this.flow.allowed_mcp_tools)
            return false;
        return this.flow.allowed_mcp_tools.some((tool) => tool.server_name === serverName && tool.tool_name === toolName);
    }
    handleToolToggle(serverName, toolName, checked) {
        if (!this.flow.allowed_mcp_tools) {
            this.flow.allowed_mcp_tools = [];
        }
        if (checked) {
            ...
            // Remove tool
            this.flow.allowed_mcp_tools = this.flow.allowed_mcp_tools.filter((tool) => !(tool.server_name === serverName && tool.tool_name === toolName));
        }
        this.requestUpdate();
    }
    getGitTrackers() {
        // Return only GitHub and GitLab trackers
        return this.trackers.filter((t) => t.tracker_type === 'github' || t.tracker_type === 'gitlab');
    }
    ...
    handleGitCloneToggle(enabled) {
        if (enabled) {
            ...
            if (gitTrackers.length === 1) {
                ...
                const triggerTracker = gitTrackers.find((t) => t.id === this.flow.trigger_event_source);
                if (triggerTracker) {
    ...
    addGitRepositoryWithTracker(trackerId) {
        if (!this.flow.git_clone_config) {
            this.flow.git_clone_config = { enabled: true, repositories: [] };
        }
        this.flow.git_clone_config.repositories =
    ...
    renderGitRepositories() {
      ...
      `;
        ...
        return html `
      ${repositories.map((repo, index) => html `
          <div
          ...
          >
            ...
            ? html `
                ...
                this.requestUpdate();
                  ...
                  >
                    ${gitTrackers.map((tracker) => html `<sl-option value=${tracker.id}
  ...
  "any_key": "any_value"
    ...
    closeAddTrackerDialog() {
        this.isAddingTracker = false;
    }
    async handleTrackerAdded(event) {
        ...
        if (this.triggerType === 'tracker' && this.trackers.length > 0) {
            ...
            const allOrganizations = await listOrganizations();
            this.organizations = allOrganizations.filter((org) => org.tracker_id === newestTracker.id);
    ...
    async handleAIModelCreated(event) {
    ...
    renderTrackerTriggerFields() {
      ...
      <div class="form-grid">
        ...
        >
          ${this.trackers.map((tracker) => html `<sl-option value=${tracker.id}>${tracker.name}</sl-option>`)}
        </sl-select>
        <sl-select
          label="Organization"
        ...
        >
          ${this.isPollingOrganizations
            ...
            : this.organizations.length === 0 &&
                ...
                : this.organizations.map((org) => html `<sl-option value=${org.id}>${org.name}</sl-option>`)}
        </sl-select>
        <sl-select
          label="Projects"
          multiple
        ...
        >
          ${this.isPollingProjects
            ...
            : (() => {
                ...
                const orgProjects = this.projects.filter((proj) => proj.organization_id === this.flow.trigger_organization_id);
                return orgProjects.map((proj) => html `<sl-option value=${proj.id}>${proj.name}</sl-option>`);
            })()}
        </sl-select>
        <sl-select
          label="Events"
        ...
        >
          ${this.getEventOptions().map((event) => html `<sl-option value=${event.value}>${event.name}</sl-option>`)}
        ...
        ${this.flow.trigger_event_types?.includes('other')
            ? html `
              <sl-input
                ...
                @sl-input=${(e) => (this.customEventType = e.target.value)}
              ></sl-input>
            `
            : ''}
      </div>
    ...
    renderEventFilters() {
        ...
        const hasFilters = this.flow.trigger_config &&
        ...
        const isMREvent = eventTypes.some((et) => et?.includes('merge_request') || et?.includes('pull_request'));
        return html `
...
__decorate([
    property()
], FlowView.prototype, "flowId", void 0);
__decorate([
    state()
], FlowView.prototype, "flow", void 0);
__decorate([
    state()
], FlowView.prototype, "isNew", void 0);
__decorate([
    state()
], FlowView.prototype, "isEditing", void 0);
__decorate([
    state()
], FlowView.prototype, "trackers", void 0);
__decorate([
    state()
], FlowView.prototype, "models", void 0);
__decorate([
    state()
], FlowView.prototype, "mcpServers", void 0);
__decorate([
    state()
], FlowView.prototype, "availableTools", void 0);
__decorate([
    state()
], FlowView.prototype, "presets", void 0);
__decorate([
    state()
], FlowView.prototype, "creationMode", void 0);
__decorate([
    state()
], FlowView.prototype, "sourcePresetId", void 0);
__decorate([
    state()
], FlowView.prototype, "organizations", void 0);
__decorate([
    state()
], FlowView.prototype, "projects", void 0);
__decorate([
    state()
], FlowView.prototype, "recentExecutions", void 0);
__decorate([
    state()
], FlowView.prototype, "isAdmin", void 0);
__decorate([
    state()
], FlowView.prototype, "triggerType", void 0);
__decorate([
    state()
], FlowView.prototype, "isAddingTracker", void 0);
__decorate([
    state()
], FlowView.prototype, "isPollingOrganizations", void 0);
__decorate([
    state()
], FlowView.prototype, "isPollingProjects", void 0);
__decorate([
    state()
], FlowView.prototype, "isAddingAIModel", void 0);
__decorate([
    state()
], FlowView.prototype, "showTestRunModal", void 0);
__decorate([
    state()
], FlowView.prototype, "_loadingReferenceData", void 0);
__decorate([
    state()
], FlowView.prototype, "formError", void 0);
__decorate([
    state()
], FlowView.prototype, "testRunPlaceholders", void 0);
__decorate([
    state()
], FlowView.prototype, "customEventType", void 0);
__decorate([
    state()
], FlowView.prototype, "filtersExpanded", void 0);
FlowView = __decorate([
