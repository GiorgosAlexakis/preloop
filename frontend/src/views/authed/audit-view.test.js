...
describe('AuditView', () => {
                        ...
                        },
                        {
                            ...
                            primary_event: {
                                ...
                                details: {
                                    ...
                                    runtime_session_id: 'runtime-session-1',
                                    api_key_name: 'Claude Workspace Token',
                                    error_detail: 'Model gateway budget exceeded: account monthly limit reached',
    ...
    it('renders expandable runtime session events and API token attribution', async () => {
        const element = document.createElement('audit-view');
        document.body.appendChild(element);
        await waitUntil(() => !element._loading, 'Audit view did not finish loading');
        ...
        const rows = Array.from(element.shadowRoot?.querySelectorAll('.primary-row') || []);
        rows[0].click();
        await element.updateComplete;
    ...
    it('renders gateway request failures with readable labels and details', async () => {
        ...
        await element.updateComplete;
        const rows = Array.from(element.shadowRoot?.querySelectorAll('.primary-row') || []);
        const gatewayRow = rows.find((row) => row.textContent?.includes('budget denied: openai/gpt-5'));
