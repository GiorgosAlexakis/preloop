...
describe('AgentDetailView', () => {
    ...
    it('renders managed agent aggregates and session activity history', async () => {
        const element = await fixture(html `<agent-detail-view agentId="agent-1"></agent-detail-view>`);
        await waitUntil(() => !element.loading && element.agent !== null, 'Agent detail view did not finish loading');
        ...
        await element.selectSession('runtime-session-2');
        await waitUntil(() => element.runtimeDetail?.session?.id === 'runtime-session-2', 'Second session detail did not load');
        await element.updateComplete;
        ...
        const sessionTwoCall = fetchStub
            .getCalls()
            .find((call) => String(call.args[0]).endsWith('/api/v1/runtime-sessions/runtime-session-2'));
...
describe('AgentDetailView', () => {
    ...
    it('renders historical aggregates for the managed agent', async () => {
        const el = await fixture(html `<agent-detail-view agentId="agent-1"></agent-detail-view>`);
