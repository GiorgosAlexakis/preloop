...
import { getAIModel, getAIModelGatewayUsageSearch, getAIModelGatewayUsageSummary, getAIModelRuntimeSessions, } from '../../../api';
import { unifiedWebSocketManager } from '../../../services/unified-websocket-manager';
import consoleStyles from '../../../styles/console-styles.css?inline';
let AIModelDetailView = class AIModelDetailView extends LitElement {
    constructor() {
        super(...arguments);
        this.modelId = '';
        this.model = null;
    ...
    connectRealtime() {
        ...
        const unsubscribers = [
            unifiedWebSocketManager.subscribe('gateway_activity', scheduleRefresh, (message) => message?.payload?.ai_model_id === this.modelId),
            unifiedWebSocketManager.subscribe('budget_health', scheduleRefresh, (message) => message?.payload?.ai_model_id === this.modelId),
            unifiedWebSocketManager.subscribe('runtime_sessions', scheduleRefresh, (message) => this.shouldRefreshForRuntimeSession(message)),
    ...
    shouldRefreshForRuntimeSession(message) {
        ...
        return (this.sessions?.items?.some((session) => session.id === runtimeSessionId) ?? false);
    }
    scheduleRefresh() {
        if (!this.modelId) {
            return;
    ...
    handleInteractionQueryChange(event) {
        this.interactionQuery = event.target.value;
    }
    async applyFilters() {
    ...
    renderSessions() {
      ...
      <div class="session-list">
        ${this.sessions.items.map((session) => html `
            ...
            : ''}
              ...
              <div>
                ${this.formatDateTime(session.last_request_at || session.last_activity_at)}
              </div>
            </div>
          `)}
      </div>
    ...
    renderSummarySection() {
      ...
      <div class="stack">
        <div class="summary-grid">
          ${this.renderStatCard('Requests', this.formatNumber(this.summary.total_requests), `${this.formatNumber(this.summary.successful_requests)} succeeded, ${this.formatNumber(this.summary.failed_requests)} failed`)}
          ${this.renderStatCard('Estimated Cost', this.formatCost(this.summary.estimated_cost), `${this.formatPercent(this.summary.successful_requests, this.summary.total_requests)} success rate`)}
          ${this.renderStatCard('Total Tokens', this.formatNumber(this.summary.token_usage.total_tokens), `${this.formatNumber(this.summary.token_usage.prompt_tokens)} prompt, ${this.formatNumber(this.summary.token_usage.completion_tokens)} completion`)}
          ${this.renderStatCard('Tracked Period', this.formatDateTime(this.summary.period_end), `${this.formatDateTime(this.summary.period_start)} to ${this.formatDateTime(this.summary.period_end)}`)}
        </div>
        <div>
          <div
            class="meta-line"
...
], AIModelDetailView.prototype, "modelId", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "model", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "summary", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "sessions", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "interactions", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "loading", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "error", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "selectedRange", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "startDate", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "endDate", void 0);
__decorate([
    state()
], AIModelDetailView.prototype, "interactionQuery", void 0);
AIModelDetailView = __decorate([
    customElement('ai-model-detail-view')
], AIModelDetailView);
export { AIModelDetailView };
