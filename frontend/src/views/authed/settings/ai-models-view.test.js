...
describe('AIModelsView', () => {
    ...
    it('links each configured model to its observability detail page', async () => {
        const element = (await fixture(html `<ai-models-view></ai-models-view>`));
        await waitUntil(() => !element.isLoading, 'AI models view did not finish loading');
        ...
        const nameLink = element.shadowRoot?.querySelector('a.model-link[href="/console/settings/ai-models/model-1"]');
        const viewButton = element.shadowRoot?.querySelector('sl-button[href="/console/settings/ai-models/model-1"]');
