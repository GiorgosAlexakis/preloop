...
let LandingView = class LandingView extends LitElement {
    constructor() {
        super(...arguments);
        this._showVideo = [false, false, false];
        ...
        this._ctaPrimary = '';
        this._ctaSecondary = '';
        this._ctaSecondaryUrl = '';
        this._getStartedTitle = '';
        this._getStartedLinkText = '';
        this._getStartedLinkUrl = '';
        this._getStartedFeatures = [];
        this._mcpSetupTitle = '';
        this._extendedDescription = '';
        this._featuresLayout = 'grid';
        this._billingEnabled = false;
        this._oauthSigninEnabled = false;
        this._productHunt = null;
        this._featuredVideo = null;
    }
    async firstUpdated() {
        await this._loadContent();
        await this._checkBillingEnabled();
    }
    async _checkBillingEnabled() {
        try {
    ...
    _loadSlottedContent(children) {
        // Read hero content from light DOM slots
        const heroTitle = children.find((el) => el.getAttribute('slot') === 'hero-title');
        const heroLead = children.find((el) => el.getAttribute('slot') === 'hero-lead');
        const ctaPrimary = children.find((el) => el.getAttribute('slot') === 'cta-primary');
        const ctaSecondary = children.find((el) => el.getAttribute('slot') === 'cta-secondary');
        const ctaSecondaryUrl = children.find((el) => el.getAttribute('slot') === 'cta-secondary-url');
        if (heroTitle)
            this._heroTitle = heroTitle.innerHTML || '';
        if (heroLead)
            this._heroLead = heroLead.textContent || '';
        if (ctaPrimary)
            this._ctaPrimary = ctaPrimary.textContent || '';
        if (ctaSecondary)
            this._ctaSecondary = ctaSecondary.textContent || '';
        if (ctaSecondaryUrl)
            this._ctaSecondaryUrl = ctaSecondaryUrl.textContent || '';
        // Read extended description from light DOM slot
        const extendedDescription = children.find((el) => el.getAttribute('slot') === 'extended-description');
        if (extendedDescription)
            this._extendedDescription = extendedDescription.textContent || '';
        // Read features layout from light DOM slot
        const featuresLayout = children.find((el) => el.getAttribute('slot') === 'features-layout');
        if (featuresLayout) {
            const layout = featuresLayout.textContent?.trim();
        ...
        const features = [];
        for (let i = 0; i < 10; i++) {
            const featureWrapper = children.find((el) => el.getAttribute('slot') === `feature-${i}`);
            if (featureWrapper) {
        ...
        const faqs = [];
        for (let i = 0; i < 20; i++) {
            const faqWrapper = children.find((el) => el.getAttribute('slot') === `faq-${i}`);
            if (faqWrapper) {
        ...
        const getStartedTitle = children.find((el) => el.getAttribute('slot') === 'get-started-title');
        const getStartedLinkText = children.find((el) => el.getAttribute('slot') === 'get-started-link-text');
        const getStartedLinkUrl = children.find((el) => el.getAttribute('slot') === 'get-started-link-url');
        if (getStartedTitle)
        ...
        for (let i = 0; i < 10; i++) {
            const featureWrapper = children.find((el) => el.getAttribute('slot') === `get-started-feature-${i}`);
            if (featureWrapper) {
        ...
        const mcpSetupTitle = children.find((el) => el.getAttribute('slot') === 'mcp-setup-title');
        if (mcpSetupTitle)
            this._mcpSetupTitle = mcpSetupTitle.textContent || '';
        // Read Product Hunt configuration from slot
        const productHuntSlot = children.find((el) => el.getAttribute('slot') === 'product-hunt');
        if (productHuntSlot) {
        ...
        const featuredVideoSlot = children.find((el) => el.getAttribute('slot') === 'featured-video');
        if (featuredVideoSlot) {
            ...
            if (enabled) {
                this._featuredVideo = {
                    ...
                    youtube_embed: featuredVideoSlot.getAttribute('data-youtube-embed') || '',
                };
            }
        }
    ...
    async _loadFromJSON() {
        ...
        this._featureSlides = features.map((f) => ({
            title: f.title || '',
            text: f.text || '',
            videoUrl: f.videoUrl || '',
            ...
            ? html `
              <section class="feature-section main-section" id="features">
                <div class="section-container text-center">
                  ...
                  >
                    ${this._featureSlides.map((slide, index) => html `
                        <sl-carousel-item>
                ...
                <section class="feature-section main-section" id="features">
                  <div class="section-container">
                    ...
                    <div class="feature-grid three-col">
                      ${this._featureSlides.map((slide) => html `
        ...
        ${html `
          <section class="feature-section main-section" id="get-started">
            ...
            ? html `<div class="feature-grid three-col">
                    ${this._getStartedFeatures.map((feature) => html `
                        <div class="feature-box">
...
__decorate([
    query('.feature-carousel')
], LandingView.prototype, "_carousel", void 0);
__decorate([
    state()
], LandingView.prototype, "_showVideo", void 0);
__decorate([
    state()
], LandingView.prototype, "_activeSlideIndex", void 0);
__decorate([
    state()
], LandingView.prototype, "_featureSlides", void 0);
__decorate([
    state()
], LandingView.prototype, "_faqs", void 0);
__decorate([
    state()
], LandingView.prototype, "_heroTitle", void 0);
__decorate([
    state()
], LandingView.prototype, "_heroLead", void 0);
__decorate([
    state()
], LandingView.prototype, "_ctaPrimary", void 0);
__decorate([
    state()
], LandingView.prototype, "_ctaSecondary", void 0);
__decorate([
    state()
], LandingView.prototype, "_ctaSecondaryUrl", void 0);
__decorate([
    state()
], LandingView.prototype, "_getStartedTitle", void 0);
__decorate([
    state()
], LandingView.prototype, "_getStartedLinkText", void 0);
__decorate([
    state()
], LandingView.prototype, "_getStartedLinkUrl", void 0);
__decorate([
    state()
], LandingView.prototype, "_getStartedFeatures", void 0);
__decorate([
    state()
], LandingView.prototype, "_mcpSetupTitle", void 0);
__decorate([
    state()
], LandingView.prototype, "_extendedDescription", void 0);
__decorate([
    state()
], LandingView.prototype, "_featuresLayout", void 0);
__decorate([
    state()
], LandingView.prototype, "_billingEnabled", void 0);
__decorate([
    state()
], LandingView.prototype, "_oauthSigninEnabled", void 0);
__decorate([
    state()
], LandingView.prototype, "_productHunt", void 0);
__decorate([
    state()
], LandingView.prototype, "_featuredVideo", void 0);
LandingView = __decorate([
