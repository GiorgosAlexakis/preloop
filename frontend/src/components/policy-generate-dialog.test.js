...
describe('PolicyGenerateDialog', () => {
    ...
    // Helpers
    // ---------------------------------------------------------------------------
    function stubGenerate(yaml, warnings = []) {
    ...
    function stubGenerateError(status, detail) {
        fetchStub.callsFake(async () => new Response(JSON.stringify({ detail }), {
            status,
            headers: { 'Content-Type': 'application/json' },
        }));
    }
    ...
    it('renders the dialog when open', async () => {
        stubGenerate('');
        const el = (await fixture(html `<policy-generate-dialog .open=${true}></policy-generate-dialog>`));
    ...
    it('generate button is disabled when prompt is empty', async () => {
        ...
        const el = (await fixture(html `<policy-generate-dialog .open=${true}></policy-generate-dialog>`));
        await el.updateComplete;
        const generateBtn = Array.from(el.shadowRoot.querySelectorAll('sl-button')).find((b) => b.textContent?.trim()?.includes('Generate'));
    ...
    it('generates policy from prompt successfully', async () => {
        ...
        await el.updateComplete;
        // Set prompt text
        el._prompt = 'require approval for bash';
        el.requestUpdate();
        await el.updateComplete;
        // Click Generate
        const generateBtn = Array.from(el.shadowRoot.querySelectorAll('sl-button')).find((b) => b.textContent?.trim()?.includes('Generate'));
        generateBtn.click();
        await waitUntil(() => el._generatedYaml !== '', 'YAML was not generated');
        await el.updateComplete;
        expect(el._generatedYaml).to.equal(yamlContent);
        expect(el._warnings).to.deep.equal(['minor warning']);
    ...
    it('shows error alert when generation fails', async () => {
        ...
        const generateBtn = Array.from(el.shadowRoot.querySelectorAll('sl-button')).find((b) => b.textContent?.trim()?.includes('Generate'));
        generateBtn.click();
        await waitUntil(() => el._error !== '', 'Error was not set');
        await el.updateComplete;
        expect(el._error).to.include('No AI model configured');
        const errorAlert = el.shadowRoot.querySelector('sl-alert[variant="danger"]');
    ...
    it('generates policy from audit logs with date range', async () => {
        ...
        // Switch to audit tab
        el._activeTab = 'audit';
        el._startDate = '2026-01-01';
        el._endDate = '2026-02-01';
        el.requestUpdate();
        await el.updateComplete;
        // Click Generate
        const generateBtn = Array.from(el.shadowRoot.querySelectorAll('sl-button')).find((b) => b.textContent?.trim()?.includes('Generate'));
        ...
        const auditCall = fetchStub
            .getCalls()
            .find((c) => String(c.args[0]).endsWith('/api/v1/policies/generate-from-audit'));
        expect(auditCall).to.exist;
        const body = JSON.parse(auditCall.args[1].body);
    ...
    it('dispatches policy-apply event when Apply Policy is clicked', async () => {
        ...
        el._generatedYaml = yamlContent;
        el.requestUpdate();
        await el.updateComplete;
        // Capture the custom event
        let eventDetail = null;
        el.addEventListener('policy-apply', ((e) => {
            eventDetail = e.detail;
        }));
        const applyBtn = Array.from(el.shadowRoot.querySelectorAll('sl-button')).find((b) => b.textContent?.trim()?.includes('Apply Policy'));
    ...
    it('resets state when dialog is closed', async () => {
        ...
        await el.updateComplete;
        // Set some state
        el._prompt = 'something';
        el._generatedYaml = 'version: "1.0"';
        ...
        el._handleClose();
        await el.updateComplete;
        expect(el._prompt).to.equal('');
        expect(el._generatedYaml).to.equal('');
        expect(el._error).to.equal('');
        expect(el._warnings).to.deep.equal([]);
