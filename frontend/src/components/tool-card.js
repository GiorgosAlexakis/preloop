...
import { getUsers, getTeams, getUserProfile, getToolApprovalCondition, fetchWithAuth, } from '../api';
...
let ToolCard = class ToolCard extends LitElement {
    constructor() {
        super(...arguments);
        this.policies = [];
        this.features = {};
    ...
    getToolArguments() {
        ...
        const properties = this.tool?.schema?.properties || this.tool?.schema?.input?.properties;
        if (!properties) {
            return [];
        }
    ...
    buildConditionExpression() {
        ...
        return this.buildSingleConditionExpression(this.conditionField, this.conditionOperator, this.conditionValue);
    }
    buildSingleConditionExpression(field, operator, value) {
    ...
    async testCelExpression() {
        ...
        try {
            ...
            const response = await fetchWithAuth(`/api/v1/tool-configurations/${this.tool.config_id}/approval-condition/test`, {
                method: 'POST',
    ...
    handleApprovalToggle() {
        ...
        if (this.tool.approval_workflow_id || this.pendingApproval) {
            this.pendingApproval = false;
            this.dispatchEvent(new CustomEvent('toggle-approval', {
            ...
            if (this.tool.approval_workflow_id) {
                ...
                if (defaultPolicy) {
                    ...
                    this.dispatchEvent(new CustomEvent('policy-selected', {
                    ...
                    this.dispatchEvent(new CustomEvent('use-default-policy', {
                        detail: { tool: this.tool },
                        bubbles: true,
                        composed: true,
                    }));
    ...
    async handleConfigureCondition() {
        if (!this.tool)
            return;
        // Reset state
        this.rawCelMode = false;
    ...
    parseSingleExpression(expression) {
        ...
        const methodMatch = expression.match(/^args\.(\w+)\.(contains|startsWith|endsWith)\(["'](.+?)["']\)$/);
        if (methodMatch) {
    ...
    handleSaveCondition() {
        ...
        this.dispatchEvent(new CustomEvent('save-condition', {
            detail: {
                tool: this.tool,
    ...
    handleClosePreloopDialog(event) {
    ...
    handlePolicyItemClick(policyId) {
        this.selectedPolicyId = policyId;
    }
    handleToggleCreatePolicy() {
    ...
    handleConfirmPolicy() {
        if (this.isCreatingPolicy) {
            ...
            if (this.newPolicyType !== 'standard' &&
            ...
            const totalApprovers = this.newPolicyApproverUserIds.length +
            ...
            if (this.hasAdvancedApprovals() &&
                ...
                alert(`Number of approvals required (${this.newPolicyApprovalsRequired}) cannot exceed the total number of potential approvers (${totalApprovers})`);
                return;
            }
            ...
            const basePolicyData = {
                ...
                approval_config: Object.keys(approvalConfig).length > 0 ? approvalConfig : null,
                is_default: this.newPolicyIsDefault,
                approver_user_ids: this.newPolicyApproverUserIds.length > 0
                    ? this.newPolicyApproverUserIds
                    : null,
                approver_team_ids: this.newPolicyApproverTeamIds.length > 0
                    ? this.newPolicyApproverTeamIds
                    : null,
                approvals_required: this.newPolicyApprovalsRequired,
                timeout_seconds: this.newPolicyTimeoutSeconds,
                escalation_user_ids: this.newPolicyEscalationUserIds.length > 0
                    ? this.newPolicyEscalationUserIds
                    : null,
                escalation_team_ids: this.newPolicyEscalationTeamIds.length > 0
                    ? this.newPolicyEscalationTeamIds
                    : null,
            };
            ...
            if (this.newPolicyType === 'ai_driven') {
                ...
                if (this.newPolicyAiFallbackBehavior === 'escalate' &&
                    this.newPolicyEscalationWorkflowId) {
            ...
            if (this.editingPolicyId) {
                ...
                this.dispatchEvent(new CustomEvent('update-policy', {
                    detail: {
                ...
                this.dispatchEvent(new CustomEvent('create-policy', {
                    detail: {
                        tool: this.tool,
                        policy: basePolicyData,
                    },
    ...
    renderSimpleConditionUI() {
      ...
      <div class="form-field">
        ...
        <sl-select
          ...
          @sl-change=${(e) => {
            ...
            const arg = this.getToolArguments().find((a) => a.name === e.target.value);
        ...
        >
          ${this.getToolArguments().map((arg) => html `
      ...
      ${this.conditionField
            ...
            <div class="form-field">
              ...
              >
                ${this.getOperatorsForType(this.getToolArguments().find((a) => a.name === this.conditionField)?.type || 'string').map((op) => html `
      ...
      >
        ${this.conditions.map((condition, index) => html `
            ...
            if (!operators.find((op) => op.value === condition.operator)) {
                this.updateCondition(index, 'operator', operators[0]?.value || 'equals');
            }
        }}
                >
                  ${this.getToolArguments().map((arg) => html `
            ...
            this.updateCondition(index, 'operator', e.target.value);
                ...
                >
                  ${this.getOperatorsForType(this.getToolArguments().find((a) => a.name === condition.field)?.type || 'string').map((op) => html `
                      <sl-option value=${op.value}>${op.label}</sl-option>
                    `)}
                </sl-select>
              </div>
          ...
          ${!this.isCreatingPolicy
            ? html `
                ...
                ? html `
                    ...
                    ? 'selected'
                    : ''}"
                                  @click=${() => this.handlePolicyItemClick(policy.id)}
                                >
...
- Credential access"
                            ...
                            <div class="form-field">
                              <label class="form-label"
                                ...
                                ${Math.round(this.newPolicyAiConfidenceThreshold * 100)}%</label
                              >
                              <sl-range
                    ...
                    ? html `
                        ...
                        this.newPolicyEscalationWorkflowId =
                            e.target.value;
                                    ...
                                    >
                                      ${this.policies
                        .filter((p) => p.approval_type === 'standard')
                        .map((p) => html `
                                    ...
                                    ${!this.newPolicyEscalationWorkflowId &&
                        this.policies.filter((p) => p.approval_type === 'standard').length > 0
                        ? html `
                                          <div
                ...
                ? html `
                          <div class="form-field">
                            ...
                            <sl-select
                              ...
                              .value=${[
                    ...this.newPolicyApproverUserIds.map((id) => `user:${id}`),
                    ...this.newPolicyApproverTeamIds.map((id) => `team:${id}`),
                ]}
                              @sl-change=${(e) => {
                    e.stopPropagation();
                    ...
                    ? html `
                                  <div class="form-field">
                                    ...
                                    <sl-select
                                      ...
                                      .value=${[
                        ...this.newPolicyEscalationUserIds.map((id) => `user:${id}`),
                        ...this.newPolicyEscalationTeamIds.map((id) => `team:${id}`),
                    ]}
                                      @sl-change=${(e) => {
                        e.stopPropagation();
                        const selected = e.target.value || [];
...
], ToolCard.prototype, "tool", void 0);
__decorate([
    property({ type: Array })
], ToolCard.prototype, "policies", void 0);
__decorate([
    property({ type: Object })
], ToolCard.prototype, "features", void 0);
__decorate([
    state()
], ToolCard.prototype, "showPreloopDialog", void 0);
__decorate([
    state()
], ToolCard.prototype, "pendingApproval", void 0);
__decorate([
    state()
], ToolCard.prototype, "selectedPolicyId", void 0);
__decorate([
    state()
], ToolCard.prototype, "isCreatingPolicy", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyName", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyDescription", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyType", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyChannel", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyUser", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyWebhookUrl", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyIsDefault", void 0);
__decorate([
    state()
], ToolCard.prototype, "editingPolicyId", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyApproverUserIds", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyApproverTeamIds", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyApprovalsRequired", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyTimeoutSeconds", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyEscalationUserIds", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyEscalationTeamIds", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyAiModel", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyAiGuidelines", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyAiConfidenceThreshold", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyAiFallbackBehavior", void 0);
__decorate([
    state()
], ToolCard.prototype, "newPolicyEscalationWorkflowId", void 0);
__decorate([
    state()
], ToolCard.prototype, "availableUsers", void 0);
__decorate([
    state()
], ToolCard.prototype, "availableTeams", void 0);
__decorate([
    state()
], ToolCard.prototype, "currentUserId", void 0);
__decorate([
    state()
], ToolCard.prototype, "showConditionConfig", void 0);
__decorate([
    state()
], ToolCard.prototype, "conditionField", void 0);
__decorate([
    state()
], ToolCard.prototype, "conditionOperator", void 0);
__decorate([
    state()
], ToolCard.prototype, "conditionValue", void 0);
__decorate([
    state()
], ToolCard.prototype, "conditions", void 0);
__decorate([
    state()
], ToolCard.prototype, "conditionCombiner", void 0);
__decorate([
    state()
], ToolCard.prototype, "rawCelMode", void 0);
__decorate([
    state()
], ToolCard.prototype, "rawCelExpression", void 0);
__decorate([
    state()
], ToolCard.prototype, "celTestResult", void 0);
__decorate([
    state()
], ToolCard.prototype, "isCelTesting", void 0);
ToolCard = __decorate([
