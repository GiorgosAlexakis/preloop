...
describe('AddTrackerModal', () => {
    ...
    describe('Add Mode', () => {
        ...
        it('renders correctly in initial add state (step 1)', async () => {
            ...
            const nameInput = element.shadowRoot?.querySelector('sl-input[name="name"]');
            expect(nameInput).to.exist;
            expect(nameInput?.value).to.be.empty;
            const typeSelect = element.shadowRoot?.querySelector('sl-select[name="type"]');
            expect(typeSelect).to.exist;
            expect(typeSelect?.value).to.equal('github');
            const nextButton = element.shadowRoot?.querySelector('sl-button[variant="primary"]');
        ...
        describe('Edit Mode', () => {
            ...
            beforeEach(async () => {
                element = await fixture(html `<add-tracker-modal .tracker=${mockTracker}></add-tracker-modal>`);
                await element.updateComplete;
            });
            it('renders correctly in initial edit state (step 1)', async () => {
                ...
                const tokenInput = element.shadowRoot?.querySelector('sl-input[name="api_key"]');
                expect(tokenInput?.value).to.equal('unchanged');
                const saveButton = element.shadowRoot?.querySelector('sl-button[variant="primary"]');
        ...
        describe('Step 1 -> Step 2 Navigation', () => {
            ...
            it('transitions to step 2 on successful validation', async () => {
                ...
                const dialog = element.shadowRoot?.querySelector('sl-dialog');
                expect(dialog?.querySelector('h2')?.textContent).to.equal('Configure Project Scope');
            });
            ...
            describe('Saving', () => {
                ...
                it('calls updateTracker and dispatches tracker-updated on save in edit mode', async () => {
                    ...
                    element = await fixture(html `<add-tracker-modal
              .tracker=${mockTracker}
            ></add-tracker-modal>`);
            ...
            describe('Cancel/Close', () => {
                it('dispatches close-modal event on cancel click', async () => {
                    element = await fixture(html `<add-tracker-modal></add-tracker-modal>`);
                    await element.updateComplete;
                    const cancelButton = element.shadowRoot?.querySelector('sl-button:not([variant="primary"])');
