...
let ConsoleHeader = class ConsoleHeader extends LitElement {
    constructor() {
        super(...arguments);
        this._user = null;
        ...
        this.shownExecutionNotifications = new Set();
        this.shownApprovalNotifications = new Set();
    }
    ...
    async loadRunningExecutions() {
        try {
            const executions = await api.getFlowExecutions();
            // Filter for running/pending executions
            this._runningExecutions = executions.filter((exec) => exec.status === 'RUNNING' || exec.status === 'PENDING');
        }
        catch (error) {
    ...
    async loadPendingApprovals() {
        try {
            ...
            this._pendingApprovals = approvals.map((approval) => ({
                id: approval.id,
    ...
    async handleApprove(approvalId, event) {
    ...
    async handleDecline(approvalId, event) {
        event.stopPropagation();
        this._processingApproval = approvalId;
        try {
    ...
    markNotificationAsRead(notificationId) {
        this._userNotifications = this._userNotifications.map((n) => n.id === notificationId ? { ...n, read: true } : n);
        // TODO: Call API to mark as read when backend supports it
        // api.markNotificationRead(notificationId);
    }
    get totalNotificationCount() {
        const unreadNotifications = this._userNotifications.filter((n) => !n.read).length;
        return (this._runningExecutions.length +
            this._pendingApprovals.length +
            unreadNotifications);
    }
    connectToFlowUpdates() {
        this.unsubscribeFlow = unifiedWebSocketManager.subscribe('flow_executions', (message) => {
            ...
            if (message.type === 'execution_started') {
                ...
                const exists = this._runningExecutions.some((exec) => exec.id === newExecution.id);
                if (!exists) {
                    this._runningExecutions = [
                        newExecution,
                        ...this._runningExecutions,
            ...
            if (message.type === 'status_update' && message.execution_id) {
                ...
                const executionIndex = this._runningExecutions.findIndex((exec) => exec.id === message.execution_id);
                if (executionIndex !== -1) {
    ...
    connectToApprovalUpdates() {
        this.unsubscribeApprovals = unifiedWebSocketManager.subscribe('approvals', (message) => {
            ...
            if (message.type === 'approval_created') {
                ...
                const exists = this._pendingApprovals.some((approval) => approval.id === newApproval.id);
                if (!exists) {
            ...
            if (message.type === 'approval_approved' ||
                ...
                this.showApprovalResolvedNotification(message.approval_request_id, message.tool_name || 'Tool', message.type);
                // Remove from pending approvals
                this._pendingApprovals = this._pendingApprovals.filter((approval) => approval.id !== message.approval_request_id);
            }
        });
    }
    connectToNotificationUpdates() {
        // TODO: Subscribe to 'notifications' WebSocket channel when backend supports it
        this.unsubscribeNotifications = unifiedWebSocketManager.subscribe('system', (message) => {
    ...
    mapMessageTypeToNotificationType(messageType) {
        const typeMap = {
    ...
    showExecutionNotification(execution) {
        ...
        if (Notification.permission !== 'granted') {
            console.log(`[Notification] Permission not granted (current: ${Notification.permission}), requesting...`);
        ...
        console.log(`[Notification] Showing start notification for ${execution.flow_name || 'Flow'} (${execution.id})`);
        try {
     ...
     */
    showExecutionFinishedNotification(execution, status) {
        if (!('Notification' in window) || Notification.permission !== 'granted') {
            console.log(`[Notification] Cannot show finished notification (permission: ${'Notification' in window ? Notification.permission : 'unsupported'})`);
            return;
        }
        console.log(`[Notification] Showing finished notification for ${execution.flow_name || 'Flow'} (${execution.id}) — status: ${status}`);
     ...
     */
    showApprovalResolvedNotification(approvalId, toolName, eventType) {
        if (!('Notification' in window) || Notification.permission !== 'granted') {
            return;
        }
        const statusMap = {
        ...
        try {
            const notification = new Notification(`${status.emoji} Approval ${status.title}`, {
    ...
    renderExecutionsSection() {
        if (this._runningExecutions.length === 0)
            return '';
        return html `
      <div class="notification-section">
        ...
        <div class="execution-list">
          ${this._runningExecutions.slice(0, 5).map((exec) => html `
              <div
    ...
    renderApprovalsSection() {
      ...
      <div class="notification-section">
        ...
        <div class="approval-list">
          ${this._pendingApprovals.slice(0, 5).map((approval) => html `
              <div
                class="approval-item"
            ...
            : ''}
                ...
                <div class="approval-actions">
                  <sl-button
                    ...
                    @click=${(e) => this.handleApprove(approval.id, e)}
                  >
                  ...
                  <sl-button
                    ...
                    ?loading=${this._processingApproval === approval.id}
                    ?disabled=${this._processingApproval !== null}
                    @click=${(e) => this.handleDecline(approval.id, e)}
                  >
    ...
    renderNotificationsSection() {
      ...
      <div class="notification-section">
        ...
        <div class="notification-list">
          ${this._userNotifications.slice(0, 5).map((notification) => html `
              <div
    ...
    render() {
        const hasContent = this._runningExecutions.length > 0 ||
        ...
        <div class="user-menu">
          ...
          <sl-dropdown distance="8">
            ...
            <sl-menu>
              ...
              <sl-menu-item
                @click=${() => Router.go('/console/settings/notification-preferences')}
              >
...
__decorate([
    state()
], ConsoleHeader.prototype, "_user", void 0);
__decorate([
    state()
], ConsoleHeader.prototype, "_runningExecutions", void 0);
__decorate([
    state()
], ConsoleHeader.prototype, "_pendingApprovals", void 0);
__decorate([
    state()
], ConsoleHeader.prototype, "_userNotifications", void 0);
__decorate([
    state()
], ConsoleHeader.prototype, "_processingApproval", void 0);
ConsoleHeader = __decorate([
