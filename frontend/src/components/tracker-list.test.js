...
describe('TrackerList', () => {
    ...
    it('renders loading state initially', async () => {
        ...
        const el = (await fixture(html `<tracker-list></tracker-list>`));
    ...
    it('renders tracker items when data is loaded', async () => {
        fetchStub.resolves(new Response(JSON.stringify(mockTrackers), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
        const el = (await fixture(html `<tracker-list></tracker-list>`));
        await waitUntil(() => el.shadowRoot?.querySelector('.tracker-grid') !== null, 'Tracker grid did not render');
    ...
    it('renders error state when fetch fails', async () => {
        ...
        const el = (await fixture(html `<tracker-list></tracker-list>`));
        await waitUntil(() => el.shadowRoot?.querySelector('sl-alert[variant="danger"]') !== null, 'Error alert did not appear');
