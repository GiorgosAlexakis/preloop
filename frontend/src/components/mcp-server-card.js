...
let MCPServerCard = class MCPServerCard extends LitElement {
    constructor() {
        super(...arguments);
    ...
    confirmDelete() {
        if (!this.server)
            return;
        this.dispatchEvent(new CustomEvent('server-deleted', {
    ...
    handleEdit() {
        if (!this.server)
            return;
        this.dispatchEvent(new CustomEvent('server-edit', {
    ...
    handleScan() {
        if (!this.server)
            return;
        this.dispatchEvent(new CustomEvent('server-scan', {
            detail: { id: this.server.id },
            bubbles: true,
            composed: true,
        }));
    ...
    handleToggleEnabled() {
        if (!this.server)
            return;
        this.dispatchEvent(new CustomEvent('server-toggle-enabled', {
            detail: {
                id: this.server.id,
...
], MCPServerCard.prototype, "server", void 0);
__decorate([
    state()
], MCPServerCard.prototype, "isConfirmingDelete", void 0);
MCPServerCard = __decorate([
