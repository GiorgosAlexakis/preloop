...
export class MessageRouter {
     ...
     */
    subscribe(topic, callback, filter) {
        const subscription = { topic, callback, filter };
        ...
        topicSubscribers.forEach((sub) => {
            if (!sub.filter || sub.filter(message)) {
                try {
                    ...
                    console.error(`[MessageRouter] Error in callback for topic ${topic}:`, error);
                }
            }
        });
