...
class ModelConfigurationBase(BaseModel):
    ...
    # owner_user_id is not directly exposed or set via API for now, managed internally or via user context
...
# Pydantic model for representing a ModelConfiguration in API responses
class ModelConfigurationResponse(ModelConfigurationBase):
    id: uuid.UUID
    owner_user_id: Optional[uuid.UUID] = Field(
        None, description="Identifier of the user who owns this configuration"
    )
    created_at: datetime
    updated_at: datetime
    ...
    # owner: Optional[UserResponseSchema] = None # Assuming a UserResponseSchema Pydantic schema exists
    # organization: OrganizationResponseSchema # Assuming an OrganizationResponseSchema Pydantic schema exists
