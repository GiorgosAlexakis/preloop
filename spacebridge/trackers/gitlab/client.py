...
class GitLabClient(TrackerInterface):
    ...
    def _parse_gitlab_issue(self, issue_data: Dict[str, Any]) -> Issue:
        ...
        priority = None
        priority_map = {
            "priority::critical": IssuePriority(id="critical", name="Critical", level=4),
    ...
    async def search_issues(
        ...
        if filter_params.status:
            ...
            if any(s.lower() == "closed" for s in filter_params.status):
                params["state"] = "closed"
            elif any(s.lower() == "opened" or s.lower() == "open" for s in filter_params.status):
                params["state"] = "opened"
    ...
    async def _get_issue_comments(self, issue_id: str) -> List[IssueComment]:
        ...
        # Get notes (comments)
        notes_path = f"/projects/{self.project_id.replace('/', '%2F')}/issues/{issue_iid}/notes"
    ...
    async def get_issue(self, issue_id: str) -> Issue:
        ...
        # Get the issue
        issue_path = f"/projects/{self.project_id.replace('/', '%2F')}/issues/{issue_iid}"
        ...
        related_path = f"/projects/{self.project_id.replace('/', '%2F')}/issues/{issue_iid}/links"
        try:
    ...
    async def create_issue(self, project_key: str, issue_data: IssueCreate) -> Issue:
        ...
        if issue_data.assignee:
            body["assignee_ids"] = [issue_data.assignee]  # GitLab requires user IDs, not usernames
    ...
    async def add_relation(
        ...
        # Add the link
        link_path = f"/projects/{self.project_id.replace('/', '%2F')}/issues/{issue_iid}/links"
        link_data = {
